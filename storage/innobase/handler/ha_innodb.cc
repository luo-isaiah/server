...
static MYSQL_THDVAR_BOOL(background_thread,
                         ...
                         "Internal (not user visible) flag "
                         "to mark background purge threads",
                         NULL, NULL, 0);
...
void innobase_destroy_background_THD(MYSQL_THD thd)
{
        /* need to close connection explicitly, the server won't do it
...
/****************************************************************//**
Ensures that if there's a concurrent online ADD INDEX,
being-indexed virtual columns are computed */
...
bool create_table_info_t::gcols_in_fulltest_or_spatial()
{
	...
	const char*		name)
{
	...
	if (high_level_read_only && !is_intrinsic_temp_table()) {
        ...
        if (gcols_in_fulltest_or_spatial()) {
                DBUG_RETURN(HA_ERR_UNSUPPORTED);
        }
...
/** Open/find the mysql table for the virtual column template
  @param[in]	thd	thread handle
@param[in,out]	table	table whose virtual column template is to be updated
	...
	dict_table_t*	table)
{
        if (table->vc_templ->mysql_table_query_id == thd_get_query_id(thd)) {
                return static_cast<TABLE*>(table->vc_templ->mysql_table);
        }
	...
	if (mysql_table == NULL) {
		...
		ret = 1; /*handler::my_eval_gcolumn_expr_with_open(
			...
			(uchar *)mysql_rec); MYSQL_VIRTUAL_COLUMNS*/
                ...
                my_bitmap_map   *old_vcol_set=tmp_use_all_columns(mysql_table, mysql_table->vcol_set);
                bool old_key_read= mysql_table->key_read;
                mysql_table->key_read= false;
                mysql_table->update_virtual_fields(VCOL_UPDATE_FOR_READ_WRITE);
                mysql_table->key_read= old_key_read;
                tmp_restore_column_map(mysql_table->vcol_set, old_vcol_set);
