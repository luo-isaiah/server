...
bool check_expression(Virtual_column_info *vcol, enum_vcol_info_type type)
{
  bool ret;
...
bool Column_definition::check(THD *thd)
{
  ...
  if (vcol_info)
  {
    ...
    if (!vcol_info->name.length)
      vcol_info->name.str= const_cast<char*>(field_name);
    if (check_expression(vcol_info, vcol_info->stored_in_db
  ...
  if (check_constraint)
  {
    if (!check_constraint->name.length)
      check_constraint->name.str= const_cast<char*>(field_name);
    if (check_expression(check_constraint, VCOL_CHECK_FIELD))
      DBUG_RETURN(1);
  }
  ...
  if (default_value)
  {
    ...
    if (!default_value->name.length)
      default_value->name.str= const_cast<char*>(field_name);
    if (check_expression(default_value, VCOL_DEFAULT))
      DBUG_RETURN(TRUE);
